...
import org.cuit.fhzheng.product.mapper.CategoryMapper;
...
//            throw new mall4cloudException("分类不存在，请刷新后重试");
    ...
    public void update(Category category) {
        ...
        if (Objects.equals(dbCategory.getCategoryId(), category.getParentId())) {
            throw new anyardOfJiangnanException("分类不能成为本身的上级分类");
        }
        existCategoryName(category);
    ...
    public void deleteById(Long categoryId) {
        ...
        if (count > 0) {
            throw new anyardOfJiangnanException("该分类在使用中，不能进行删除操作");
        }
    ...
    private void existCategoryName(Category category) {
        ...
        if (countByName > 0) {
            throw new anyardOfJiangnanException("分类名已存在，请重新输入");
        }
    }
}

  ...
  <view class="index">
    ...
    <view class="search-box">
      <view class="search" @tap="toSrearch">
        ...
        <view class="text">点击这里可以搜索商品或者店铺</view>
      </view>
    </view>
...
export default {
  ...
  onShow() {
    ...
    if (this.isWechat) { // H5-微信环境
      if (!tempuid && !token) {
        ...
        this.login('/anyard-of-jiangnan-auth/ua/social/mp', code) // 先请求微信公众号登录接口
      } else {
        this.queryDataGroup()
      }
    } else {
  ...
  methods: {
    ...
    getIndexImgs() {
      uni.showLoading()
      var params = {
        url: '/anyard-of-jiangnan-multishop/ua/index_img/list',
        method: 'GET',
        data: {
    ...
    getSearchList() {
      ...
      const params = {
        url: '/anyard-of-jiangnan-search/ua/search/simple_page',
        method: 'GET',
    ...
    getPlatformCategoryList() {
      uni.showLoading()
      const params = {
        url: '/anyard-of-jiangnan-product/ua/category/category_list',
        method: 'GET',
        data: {
          shopId: 0,
          parentId: 0
